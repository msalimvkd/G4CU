//g++ recons_noise.cpp `root-config --cflags --libs` -o reconsNoise
...
const double sigma_meas = 0.01;  // 0.01 cm resolution
const double sigma2 = sigma_meas * sigma_meas;
// Fixed distance between two detector layers (in cm)
...
// Kalman filter class using the state vector: (x, y, dx/dz, dy/dz, q/p)
class KalmanFilter {
public:
    ...
    // Initialize the filter with the first two hits (if available).
    // If only one hit is available, use default slopes (zero) and a nominal q/p.
    ...
    // Here, we use the fixed layer spacing (dz = 10 cm).
    void Predict() {
        ...
        TMatrixD newCov = F * cov * TMatrixD(TMatrixD::kTransposed, F) + Q;   //Pk=FPk-1Ft + Q
...
int main(int argc, char **argv) {
  ...
  TH1D *hMomentumResidual = new TH1D("xpositionResidual", "Residual Distribution from Two Methods", 200, -10, 10);
  ...
  hMomentumResidual->SetLineWidth(2);
    ...
    for (auto &entry : eventHits) {
        ...
        for (size_t i = 1; i < hits.size(); i++) {
            ...
            hMomentumResidual->Fill(hits[i].x-kf.state[0]);
